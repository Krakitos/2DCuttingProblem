...
public class GeneticAlgorithm extends GeneticAlgorithmEventDispatcher {
    ...
    public Chromosome start(@NotNull final Population population){
        ...
        while(!stop.isReached()){
            ...
            if(RANDOM_GEN.nextInt(100) <= crossoverOperator.crossoverRate() * 100){
                ...
                crossover.forEach(c -> {
                    if (RANDOM_GEN.nextInt(100) <= mutationOperator.mutatationRate() * 100) {
                        final Chromosome mutated = mutationOperator.mutate(c);
                        final Fitness mutatedFitness = fitnessEvaluator.computeFitness(mutated);
        ...
        LOGGER.info("Done after {} iterations ({})", iterations.get(),
